...
generate_news_matrix_t_grid <- function(data_path = "news.csv", n_eval = 10, n_tune = 2) {
  # 读取并归一化数据
...
# 样本量范围
sample_sizes <- seq(1000, 2001, 1000)
...
for (n in sample_sizes) {
  ...
  for (rep in 1:20) {
    ...
    #best_h = 0.1
    # 计算不同方法的权重
    formula <- as.formula(paste("t ~", paste(colnames(data_df)[2:(ncol(data_df) - 399)], collapse = " + ")))
    gps_est <- safe_weightit(formula, method = "glm", data = data_df, stabilize = TRUE)
    cbps_est <- safe_weightit(formula, method = "cbps", data = data_df, over = FALSE, stabilize = TRUE)
    gbm_est <- safe_weightit(formula, method = "gbm", data = data_df, stabilize = TRUE, criterion = "p.mean")
    #ebal_est <- safe_weightit(formula, method = "ebal", data = data_df, stabilize = TRUE)
    #bart_est <- safe_weightit(formula, method = "bart", data = data_df, stabilize = TRUE, n.trees = 250)
    indep_weights <- independence_weights(data_df$t, data_df %>% select(x1:x100))
    
    # 将所有大于500的权重设为500
    gps_est$weights[gps_est$weights > 5000] <- 5000
    cbps_est$weights[cbps_est$weights > 5000] <- 5000
    gbm_est$weights[gbm_est$weights > 5000] <- 5000
    #ebal_est$weights[ebal_est$weights > 500] <- 500
    #bart_est$weights[bart_est$weights > 500] <- 500
    indep_weights$weights[indep_weights$weights > 500] <- 5000
  ...
  # 计算IRMSE
  gps_squared_errors <- rowMeans((gps_results - t_grid[2, 2001:nrow(data_matrix)])^2)
  gps_DR_squared_errors <- rowMeans((gps_DR_results - t_grid[2,2001:nrow(data_matrix) ])^2)
  cbps_squared_errors <- rowMeans((cbps_results - t_grid[2,2001:nrow(data_matrix) ])^2)
  gbm_squared_errors <- rowMeans((gbm_results - t_grid[2, 2001:nrow(data_matrix)])^2)
  #ebal_squared_errors <- rowMeans((ebal_results - t_grid[2,2001:nrow(data_matrix) ])^2, na.rm = TRUE) 
  #bart_squared_errors <- rowMeans((bart_results - t_grid[2,2001:nrow(data_matrix) ])^2)
  indep_squared_errors <- rowMeans((indep_results - t_grid[2,2001:nrow(data_matrix) ])^2)
  indep_DR_squared_errors <- rowMeans((indep_DR_results - t_grid[2, 2001:nrow(data_matrix)])^2)
  ...
  #ebal_IRMSE <- mean(ebal_rmse, na.rm = TRUE)
