...
cross_validate_h <- function(data_df, folds = 5) {
  ...
  for (i in 1:length(h_values)) {
    ...
    for (fold in 1:folds) {
      ...
      # 计算不同方法的权重
      gps_est <- safe_weightit(formula, method = "gps", data = train_fold, stabilize = TRUE)
...
for (n in sample_sizes) {
  ...
  for (rep in 1:20) {
    ...
    # 计算不同方法的权重
    #formula <- as.formula(paste("t ~", paste(colnames(data_df)[2:(ncol(data_df) - 199)], collapse = " + ")))
    ...
    indep_weights <- independence_weights(data_df$t, data_df %>% select(x1:x300))
    ...
    # 使用dr_pseudo_outcome_value_function计算indep_DR方法的value_function
