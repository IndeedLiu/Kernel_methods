...
for (n in sample_sizes) {
  ...
  # cbps_results <- data.frame(matrix(ncol = 0, nrow = 1659))
  gbm_results <- data.frame(matrix(ncol = 0, nrow = 3659))
  # ebal_results <- data.frame(matrix(ncol = 0, nrow = 3659))
  ...
  for (rep in 1:1) {
    ...
    # 计算不同方法的权重
    formula <- as.formula(paste("t ~", paste(colnames(data_df)[2:(ncol(data_df) - 2001)], collapse = " + ")))
    ...
    # indep_weights <- independence_weights(data_df$t, data_df %>% select(x1:x2000))
  ...
  # cbps_squared_errors <- rowMeans((cbps_results - t(t_grid[2, 6001:nrow(data_matrix)]))^2)
  gbm_squared_errors <- rowMeans((gbm_results - t(t_grid[2, 6001:nrow(data_matrix)]))^2)
  # ebal_squared_errors <- rowMeans((ebal_results - t(t_grid[2, 6001:nrow(data_matrix)]))^2, na.rm = TRUE) 
